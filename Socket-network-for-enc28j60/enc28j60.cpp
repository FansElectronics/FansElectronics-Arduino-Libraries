...
byte initialize (word size, const byte* macaddr, byte csPin) {
    bufferSize = size;
...
void enc28j60PacketSend(uint16_t len, uint8_t* buffer) {
...
uint16_t enc28j60PacketReceive(uint16_t maxlen, uint8_t* buffer) {
    word len = 0;
...
void copyout (byte page, const byte* data) {
...
void copyin (byte page, byte* data) {
    word destPos = SCRATCH_START + (page << SCRATCH_PAGE_SHIFT);
    if (destPos < SCRATCH_START || destPos > SCRATCH_LIMIT - SCRATCH_PAGE_SIZE)
        return;
...
byte peekin (byte page, byte off) {
    byte result = 0;
...
uint8_t* tcpOffset () { return buffer + 0x36; }